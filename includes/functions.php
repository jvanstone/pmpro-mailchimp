...
function pmpromc_additional_lists_on_checkout() {
	...
	/*******
	 * 
	 *   Section to edit the Sign-up for a mailing list. 
	 * 
	 * 
	 * 
	 * 
	...
	?>
	<div id="pmpro_mailing_lists" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php echo( $display_modifier ); ?>>
		<h3>
		...
		<div class="col-12 p-2 ml-5">
				...
				foreach ( $additional_lists_array as $key => $additional_list ) {
					$count++;
					?>
					<input type="checkbox" class="form-check-input" type="checkbox"  id="additional_lists_<?php echo( $count ); ?>" name="additional_lists[]" value="<?php echo( $additional_list->id ); ?>" 
							<?php
							...
							?>
							/>
					<label class="form-check-label" for="additional_lists_<?php echo( $count ); ?>" class="pmpro_normal pmpro_clickable"><?php echo( $additional_list->name ); ?></label><br/>
					<?php
				}
				?>
